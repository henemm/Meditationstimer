...
**Example Bug (Workout double-logging):**
```swift
...
// Result: HKWorkoutBuilder.finishWorkout() called twice
```
...
if isPaused {
    if currentPhase.isWork {
        ...
        // REST phase paused: show ONLY next exercise
...
**Problem:** "Als naechstes" text not consistently displayed with correct fonts.

**Solution:** Split text into separate views:
```swift
VStack(spacing: 4) {
    Text(nextInfo.prefix)  // "Als naechstes"
        .font(.caption)
        .foregroundStyle(.secondary)

    Text(nextInfo.name)  // Exercise name
        .font(.headline)
}
```
...
**Example:**
- Original: Calendar tap -> show tooltip
- Wrong change: Calendar tap -> open logging sheet (breaking!)
- Correct: Ask "Calendar tap shows tooltip - should I change this?"
