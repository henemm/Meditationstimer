...
//  Migrated to TrackerManager on 31.01.2026.
//

import XCTest
@testable import Lean_Health_Timer

/// Tests for NoAlc streak calculation with Joker system.
/// Uses TrackerManager.calculateNoAlcStreakFromHealthKit() which implements
/// the same logic as the deprecated NoAlcManager.calculateStreakAndRewards().
final class NoAlcStreakTests: XCTestCase {
    ...
    /// Helper to build alcoholDays dictionary using TrackerLevel
    private func buildAlcoholDays(_ entries: [(daysAgo: Int, levelKey: String)]) -> [Date: TrackerLevel] {
        var dict: [Date: TrackerLevel] = [:]
        for entry in entries {
            if let level = TrackerLevel.noAlcLevels.first(where: { $0.key == entry.levelKey }) {
                dict[daysAgo(entry.daysAgo)] = level
            }
        }
        return dict
    }
    ...
    func testStreakWithAllSteadyDays() {
        ...
        let result = TrackerManager.calculateNoAlcStreakFromHealthKit(alcoholDays: alcoholDays, calendar: calendar)

        XCTAssertEqual(result.currentStreak, 7, "7 Steady days should give streak of 7")
        XCTAssertEqual(result.availableRewards, 1, "7 Steady days should earn 1 Joker")
    }
    ...
    func testStreakBreaksOnEasyWithoutJoker() {
        ...
        XCTAssertEqual(result.currentStreak, 0, "Easy day without Joker should break streak")
        XCTAssertEqual(result.availableRewards, 0, "No Jokers should be available")
    }
    ...
    func testStreakBreaksOnWildWithoutJoker() {
        ...
        XCTAssertEqual(result.currentStreak, 0, "Wild day without Joker should break streak")
        XCTAssertEqual(result.availableRewards, 0, "No Jokers should be available")
    }
    ...
    func testJokerHealsEasyDay() {
        ...
        var entries: [(daysAgo: Int, levelKey: String)] = []
        ...
        XCTAssertEqual(result.currentStreak, 8, "Joker should heal Easy day, streak continues to 8")
        XCTAssertEqual(result.availableRewards, 0, "Joker should be consumed (1 earned - 1 used = 0)")
    }
    ...
    func testJokerHealsWildDay() {
        ...
        XCTAssertEqual(result.currentStreak, 8, "Joker should heal Wild day, streak continues to 8")
        XCTAssertEqual(result.availableRewards, 0, "Joker should be consumed")
    }
    ...
    func testGapRequiresJoker() {
        ...
        XCTAssertEqual(result.currentStreak, 1, "Gap without Joker should break streak, only today counts")
        XCTAssertEqual(result.availableRewards, 0, "No Jokers available")
    }
    ...
    func testJokerHealsGap() {
        ...
        XCTAssertEqual(result.currentStreak, 9, "Joker should heal gap, streak = 7 + 1 (healed gap) + 1 (today)")
        XCTAssertEqual(result.availableRewards, 0, "Joker consumed for gap (1 earned - 1 used)")
    }
    ...
    func testMultipleGapsNeedMultipleJokers() {
        ...
        XCTAssertEqual(result.currentStreak, 17, "2 Jokers should heal 2 gaps")
        XCTAssertEqual(result.availableRewards, 0, "Both Jokers consumed (2 earned - 2 used)")
    }
    ...
    func testTooManyGapsBreaksStreak() {
        ...
        XCTAssertEqual(result.currentStreak, 1, "Second gap without Joker breaks streak, only today counts")
        XCTAssertEqual(result.availableRewards, 0, "Joker was consumed for first gap")
    }
    ...
    func testMaxThreeJokersOnHand() {
        ...
        XCTAssertEqual(result.currentStreak, 28, "28 Steady days = streak 28")
    ...
    func testJokerCapResetsAfterUse() {
        ...
        XCTAssertEqual(result.currentStreak, 31, "Full streak: 21 + 3 (healed) + 7 = 31")
        XCTAssertEqual(result.availableRewards, 1, "New Joker earned after using all 3")
    }
    ...
    func testEarnJokerBeforeConsume() {
        ...
        XCTAssertEqual(result.currentStreak, 7, "Day 7 Easy should be healed: earn Joker first, then use it")
    ...
    func testTodayNotLoggedIsIgnored() {
        ...
        XCTAssertEqual(result.currentStreak, 1, "Today not logged should be ignored, streak counts yesterday")
    }
    ...
    func testTodayLoggedCounts() {
        ...
        XCTAssertEqual(result.currentStreak, 2, "Both yesterday and today should count")
    }
    ...
    func testEmptyAlcoholDays() {
        ...
        XCTAssertEqual(result.currentStreak, 0, "No entries = no streak")
    ...
    func testOnlyTodayLogged() {
        ...
        XCTAssertEqual(result.currentStreak, 1, "Only today logged = streak 1")
