                ...
                if announceDelay > 0.001 {
                    schedule(announceDelay) { [speakExerciseNames] in
                        if nextRound == self.cfgRepeats {
                            ...
                            // Normal round announcement: "Round X of Y"
                            if speakExerciseNames {
                                let text = String(format: NSLocalizedString("Round %d of %d", comment: "TTS for round X of Y in free workout"), nextRound, self.cfgRepeats)
                                self.sounds.speak(text)
                            } else {
                                self.sounds.playRound(nextRound, of: self.cfgRepeats)
                            }
                            print("[Workout] Round announcement: \(nextRound) of \(self.cfgRepeats)")
                        }
                    }
                }
            }
